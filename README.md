...
syscalls: `page_id * PAGE_SIZE` byte offsets, `allocate_page`, and `write_page`.
Data pages are NO-FORCE - durability rides on the WAL, and `sync()` forces the
file to disk at checkpoint and on close rather than fsync'ing every page write.
Tracks read/write counters.
...
- **How we built it.**
  - *Write path:* `put`/`delete` first update an in-memory **MemTable**. Deletes
    are tombstones. The standalone engine rotates and flushes a sorted **L0
    SSTable** (sparse index + Bloom filter) once the MemTable hits `memtable_limit`;
    in the SQL backend auto-flush is off, so the MemTable instead flushes to L0 at
    checkpoint/close.
...
- **SQL integration.** We made LSM the default SQL table backend, not a separate
  ...
  the end-to-end path.
- **What we observed.** See Section 10 - LSM gives about 2.7x write throughput
...
report the median of 9 local trials and write the min/max ranges to
...
**Analysis.**
- **Write throughput (LSM ~2.7x faster).** LSM inserts mostly touch the MemTable,
  then flush sorted runs; the heap+B+ tree baseline pays for heap insertion plus
  primary-key index work per row. The gap is ~2.7x (not larger) because the heap
  follows the same NO-FORCE rule as the rest of the engine: data pages are not
  fsync'd per write, only at checkpoint/close, so the baseline isn't penalized by
  a synchronous disk flush on every page.
- **Read latency (LSM ~7.8x slower on hits).** The slower reads were also
  ...
  misses: 23,270 SSTable reads were skipped across 5,000 negative lookups.
