...
class _Parser:
    ...
    def _name(self):
        # identifiers must be words; guards a truncated statement handing us None
        # (EOF) or stray punctuation.
        kind, val = self.next()
        if kind != "word":
            raise ParseError(f"expected a name, got {val!r}")
        return val
    ...
    def _columns(self):
        ...
        while True:
            kind, val = self.next()
            if val != "*" and kind != "word":
                raise ParseError(f"expected a column name or '*', got {val!r}")
            cols.append(val)
            kind, val = self.peek()
            if val == ",":
                self.next()
...
def parse(sql: str):
    ...
    node = p.parse()
    if p.i < len(p.t):
        raise ParseError(f"unexpected trailing tokens: {p.t[p.i:]}")
    return node
