...
the pager. Insert does proper node splits; delete is lazy (remove the leaf
entry, collapse a single-child root) with no borrow/merge rebalancing.
...
class BPlusTree:
    ...
    def delete(self, key) -> bool:
        ...
        Lazy deletion: drop the leaf entry and collapse a single-child root.
        Nodes are allowed to underflow; the tree stays correct and ordered.
        """
        # TODO: no borrow/merge rebalancing on underflow. Leaves can end up
        # sparse after many deletes, but that's fine for this workload.
        leaf = self._find_leaf(key)
        i = bisect_left(leaf.keys, key)
