...
def temp_db(**kwargs):
    """Open a Database in a throwaway dir and always tear it down afterwards."""
    d = temp_db_dir()
...
def assert_raises(exc, substr, fn, *args, **kwargs):
    """Call fn and assert it blows up with `exc` whose message contains `substr`."""
    try:
        fn(*args, **kwargs)
    except exc as e:
        assert substr in str(e), f"expected {substr!r} in {str(e)!r}"
        return e
    raise AssertionError(f"expected {exc.__name__} containing {substr!r}, nothing raised")
...
def test_sql_and_optimizer():
    with temp_db() as db:
        ...
        # equality on the PK should pick the index; on a non-key column it can't.
...
def test_crash_is_terminal_but_safe_to_close():
    ...
    db.crash()
    db.close()
    # once crashed, a checkpoint must refuse rather than half-write state.
    assert_raises(RuntimeError, "crashed", db.checkpoint)
    shutil.rmtree(d)
...
def test_checkpoint_rejects_active_transactions():
    with temp_db() as db:
        ...
        assert_raises(RuntimeError, "active transaction", db.checkpoint)
        db.abort(txn)  # clear the loser so the context manager can close cleanly
...
def test_public_operations_fail_clearly_after_crash():
    ...
    for op in (lambda: db.begin(),
        ...
        assert_raises(RuntimeError, "crashed", op)
...
def test_close_rejects_active_transactions():
    ...
    assert_raises(RuntimeError, "active transaction", db.close)
    db.abort(txn)
    db.close()
...
def test_lsm_compaction_persists_manifest_before_deleting_inputs():
    ...
    try:
        assert_raises(RuntimeError, "obsolete SSTable cleanup", lsm.compact)
    finally:
...
def test_deadlock_detection():
    # two transactions grabbing the same rows in opposite order -> one must lose.
    with temp_db() as db:
        ...
        def worker(name, first, second):
            ...
            try:
                tbl.update_by_key(txn, first, [first, 1]); barrier.wait(); time.sleep(0.05)
                tbl.update_by_key(txn, second, [second, 2]); db.commit(txn)
                results[name] = "C"
...
def test_scan_blocks_phantom_inserts_until_commit():
    for storage_kind in ("heap", "lsm"):
        with temp_db(storage_kind=storage_kind) as db:
            ...
            thread.start()
            time.sleep(0.05)
            # the reader's range lock should keep the phantom insert parked.
...
def test_heap_index_rebuilds_after_reopen():
    ...
    # the PK index must come back on reopen, so a clashing insert still rejects.
    assert_raises(ValueError, "duplicate primary key",
...
def test_heap_abort_rollback_handles_relocated_updates():
    with temp_db(storage_kind="heap") as db:
        db.execute("CREATE TABLE h (id INT PRIMARY KEY, payload TEXT)")
        db.execute("INSERT INTO h VALUES (1, 'seed')")
        table = db.get_table("h")

        # growing the row twice forces a relocation; abort must still restore the seed.
        txn = db.begin()
...
# The next six tests all prove the same invariant: a WAL append must succeed
# before the in-memory/on-disk state is mutated. The heap and LSM engines verify
# that differently (heap: evict + crash + reopen; LSM: inspect + abort in place),
# so the shared skeleton lives here and each test below is a one-line call.
#
# `patch` turns a WAL method into one that raises `message` for `victim` and
# delegates otherwise; the assorted signatures are why it's passed in per case.

def _heap_wal_fault(log_attr, patch, message, run_op, victim, survives):
    """Heap path: the loser mutation must vanish across a crash + recovery."""
    d = temp_db_dir()
    db = Database(d, storage_kind="heap")
    db.execute("CREATE TABLE h (id INT PRIMARY KEY, payload TEXT)")
    db.execute("INSERT INTO h VALUES (1, 'seed')")
    db.checkpoint()
    table = db.get_table("h")
    original = getattr(db.wal, log_attr)

    def boom(*args):
        # evicting first proves the dirty page can't sneak to disk ahead of the log.
        force_buffer_eviction(db)
        raise RuntimeError(message)

    setattr(db.wal, log_attr, patch(original, boom, victim))
    txn = db.begin()
    assert_raises(RuntimeError, message.split("simulated ")[1], run_op, table, txn)
    db.crash()

    reopened = Database(d)
    assert reopened.execute(f"SELECT payload FROM h WHERE id = {victim}").rows == survives
    reopened.close(); shutil.rmtree(d)


def _lsm_wal_fault(log_attr, patch, message, run_op, victim, seed, expect):
    """LSM path: the memtable must be untouched, so an abort leaves the seed intact."""
    d = temp_db_dir()
    db = Database(d)
    ...
    txn = db.begin()
    assert_raises(RuntimeError, message.split("simulated ")[1], run_op, table, txn)
    assert table.get_by_key(txn, victim) == expect
    db.abort(txn)
    db.close(); shutil.rmtree(d)


# Each `patch` matches one WAL method's argument shape.
def _patch_insert(orig, boom, victim):
    def wrapped(txn_id, table_name, key, row):
        return boom() if key == victim else orig(txn_id, table_name, key, row)
    return wrapped


def _patch_update(orig, boom, victim):
    def wrapped(txn_id, table_name, key, old, row):
        return boom() if key == victim else orig(txn_id, table_name, key, old, row)
    return wrapped


def _patch_delete(orig, boom, victim):
    def wrapped(txn_id, table_name, key, old):
        return boom() if key == victim else orig(txn_id, table_name, key, old)
    return wrapped


def test_heap_insert_logs_before_dirty_page_can_flush():
    _heap_wal_fault("log_insert", _patch_insert, "simulated WAL append failure",
                    lambda t, txn: t.insert(txn, [2, "unlogged_loser"]),
                    victim=2, survives=[])


def test_heap_update_logs_before_dirty_page_can_flush():
    _heap_wal_fault("log_update", _patch_update, "simulated WAL update append failure",
                    lambda t, txn: t.update_by_key(txn, 1, [1, "unlogged_update"]),
                    victim=1, survives=[("seed",)])


def test_heap_delete_logs_before_dirty_page_can_flush():
    _heap_wal_fault("log_delete", _patch_delete, "simulated WAL delete append failure",
                    lambda t, txn: t.delete_by_key(txn, 1),
                    victim=1, survives=[("seed",)])


def test_lsm_insert_logs_before_mutating_memtable():
    _lsm_wal_fault("log_insert", _patch_insert, "simulated WAL append failure",
                   lambda t, txn: t.insert(txn, [2, "unlogged_loser"]),
                   victim=2, seed=False, expect=None)


def test_lsm_update_logs_before_mutating_memtable():
    _lsm_wal_fault("log_update", _patch_update, "simulated WAL update append failure",
                   lambda t, txn: t.update_by_key(txn, 1, [1, "unlogged_update"]),
                   victim=1, seed=True, expect=(1, "seed"))


def test_lsm_delete_logs_before_mutating_memtable():
    _lsm_wal_fault("log_delete", _patch_delete, "simulated WAL delete append failure",
                   lambda t, txn: t.delete_by_key(txn, 1),
                   victim=1, seed=True, expect=(1, "seed"))
...
def test_failed_commit_is_not_durable_after_crash():
    ...
    try:
        assert_raises(OSError, "commit fsync failure", db.commit, txn)
        db.crash()
    finally:
...
def test_benchmark_harness_cleans_up_constructor_failures():
    ...
    try:
        assert_raises(RuntimeError, "constructor failed", bench.run, 10)
        # the harness should have swept its temp dir even though the ctor blew up.
