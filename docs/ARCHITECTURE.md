...
|---|---|---|
| Disk I/O | `minidb/storage/disk_manager.py` | page-grained read/write/allocate, `sync()` (fsync) at checkpoint/close |
...
committed transactions after the last checkpoint and unconditionally undoes loser
operations in reverse log order; undo is idempotent, so it is safe whether or not
the loser's pages ever reached disk. Table
